...
class DaySeven {
    ...
    companion object {
        ...
        private val searchedBags = mutableListOf<String>()

        fun firstPuzzle() {
            updateBagsListForSpecificBag("shiny gold", input)
            println("Result: ${searchedBags.size - 1}")
        }

        private fun updateBagsListForSpecificBag(bag: String, input: List<String>) {
            searchedBags.add(bag)

            for (rule in input) {
                val split = rule.split(" bags contain ")

                if (split[1].contains(bag)) {
                    if (!searchedBags.contains(split[0])) {
                        updateBagsListForSpecificBag(split[0], input)
                    }
                }
            }
        }
        ...
        fun testFirstPuzzle() {
            ...
            val bag = "shiny gold"
            searchedBags.add(bag)
            updateBagsListForSpecificBag(bag, input)
            val resultInfo = if (searchedBags.size - 1 == 4) "OK" else "ERROR"
            println("Result: $resultInfo")
        }

        fun secondPuzzle() {
            val outerBag = "shiny gold bags"
            val result = getInsideBagsCount(outerBag, input)
            println("Result: $result")
        }

        private fun getInsideBagsCount(outerBag: String, input: List<String>): Int {
            var insideBagsCount = 0
            for (rule in input) {
                val split = rule.split(" contain ", ", ")
                val bagRules = split.minus(split[0])

                var sum = 0
                if (split[0].contains(outerBag)) {
                    for (bagRule in bagRules) {
                        if (bagRule == "no other bags") {
                            return 0
                        }
                        val bagCount = bagRule.substring(0, 1).toInt()
                        val bagName = bagRule.substring(2)
                        sum += bagCount
                        insideBagsCount += bagCount * getInsideBagsCount(bagName, input)
                    }
                }
                insideBagsCount += sum
            }
            return insideBagsCount
        }
        
        fun testSecondPuzzle() {
            val input = listOf(
            ...
            val outerBag = "shiny gold"
            val result = getInsideBagsCount(outerBag, input)
            println("Result: $result, expecting: 126")
        }
    }
}