...
//            "cid"
        ...
        val validEyeColors = "amb blu brn gry grn hzl oth"

        fun firstPuzzle() {
            var validPassports = passportsInfo.size

            for (passport in passportsInfo) {
            ...
            println("Result: $validPassports")
        }

        fun secondPuzzle() {
            var validPassports = passportsInfo.size

            loop@ for (passport in passportsInfo) {
                val split = passport.split(" ")
                var fieldsCount = 0

                for (field in split) {
                    val keyValue = field.split(":")
                    val fieldName = keyValue[0]
                    val value = keyValue[1]

                    if (fieldName.contains(requiredFields[0])) {
                        fieldsCount++
                        if (value.length != 4 || value.toInt() < 1920 || value.toInt() > 2002) {
                            validPassports--
                            continue@loop
                        }
                    }
                    if (fieldName.contains(requiredFields[1])) {
                        fieldsCount++
                        if (value.length != 4 || value.toInt() < 2010 || value.toInt() > 2020) {
                            validPassports--
                            continue@loop
                        }
                    }
                    if (fieldName.contains(requiredFields[2])) {
                        fieldsCount++
                        if (value.length != 4 || value.toInt() < 2020 || value.toInt() > 2030) {
                            validPassports--
                            continue@loop
                        }
                    }
                    if (fieldName.contains(requiredFields[3])) {
                        fieldsCount++
                        val metric = value.substring(value.length - 2, value.length)
                        val metricValue = value.substring(0, value.length - 2)
                        if ((metric != "cm" && metric != "in") ||
                            (metric == "cm" && (metricValue.toInt() < 150 || metricValue.toInt() > 193)) ||
                            (metric == "in" && (metricValue.toInt() < 59 || metricValue.toInt() > 76))) {
                            validPassports--
                            continue@loop
                        }
                    }
                    if (fieldName.contains(requiredFields[4])) {
                        fieldsCount++
                        if (value.length != 7 || !value.contains(Regex("^#([a-fA-F0-9]{6})$"))) {
                            validPassports--
                            continue@loop
                        }
                    }
                    if (fieldName.contains(requiredFields[5])) {
                        fieldsCount++
                    ...
                    if (fieldName.contains(requiredFields[6])) {
                        fieldsCount++
                        if (value.length != 9) {
                            validPassports--
                            continue@loop
                ...
                if (fieldsCount != requiredFields.size) {
                    validPassports--
                }
            }
